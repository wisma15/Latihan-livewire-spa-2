...
class Keranjang extends Component
{
    ...
    public function destroy($id)
    {
        ...
        if(!empty($pesanan_detail)){
            ...
            session()->flash('message', 'Pesanan terhapus');
        }
    }

    public function keHistory(){
    ...
    public function render()
    {
        if (Auth::user()) {
            ...
            if ($this->pesanan) {
                $this->pesanan_details = PesananDetail::with('post')->where('pesanan_id', $this->pesanan->id)->get();
            }
        }

        return view('livewire.keranjang', [
            'pesanan' => $this->pesanan,
            'pesanan_detail' => $this->pesanan_details,
        ])
            ->layout('layouts/home');
    }
}
